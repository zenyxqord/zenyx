...
#if defined(NDEBUG)
# error "Zenyx Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Zenyx
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        nSubsidyBase = 4570186;
    } else if (nPrevHeight <= 720 ) {
        nSubsidyBase = 17;
    } else if (nPrevHeight <= 5740) {
        nSubsidyBase = 2400;
    } else if (nPrevHeight <= 18100) {
        nSubsidyBase = 2000;
    } else if (nPrevHeight <= 32500) {
        nSubsidyBase = 1700;
    } else if (nPrevHeight <= 46900) {
        nSubsidyBase = 1000;
    } else if (nPrevHeight <= 64180) {
        nSubsidyBase = 800;
    } else if (nPrevHeight <= 92980) {
        nSubsidyBase = 600;
    } else if (nPrevHeight <= 121780) {
        nSubsidyBase = 400;
    } else if (nPrevHeight <= 150580) {
        nSubsidyBase = 300;
    } else if (nPrevHeight <= 179380) {
        nSubsidyBase = 240;
    } else if (nPrevHeight <= 208180) {
        nSubsidyBase = 200;
    } else if (nPrevHeight <= 236980) {
        nSubsidyBase = 170;
    } else if (nPrevHeight <= 265780) {
        nSubsidyBase = 120;
    } else if (nPrevHeight <= 328780) {
        nSubsidyBase = 80;
    } else if (nPrevHeight <= 400780) {
        nSubsidyBase = 60;
    } else if (nPrevHeight <= 472780) {
        nSubsidyBase = 40;
    } else if (nPrevHeight <= 544780) {
        nSubsidyBase = 24;
    } else if (nPrevHeight <= 616780) {
        nSubsidyBase = 20;
    } else if (nPrevHeight <= 688780) {
        nSubsidyBase = 17;
    } else if (nPrevHeight <= 760780) {
        nSubsidyBase = 10;
    } else if (nPrevHeight <= 832780) {
        nSubsidyBase = 8;
    } else if (nPrevHeight <= 904780) {
        nSubsidyBase = 6;
    } else if (nPrevHeight <= 1048780) {
        nSubsidyBase = 4;
    } else if (nPrevHeight <= 1120780) {
        nSubsidyBase = 2;
    } else if (nPrevHeight <= 1192780) {
        nSubsidyBase = 1;
    } else {
	    nSubsidyBase = 0;
    }
...
static int64_t nTimeZenyxSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(ZENYX): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // ZENYX : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(ZENYX): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(ZENYX): spork is off, skipping transaction locking checks\n");
    }
    ...
    // ZENYX : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ZENYX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(ZENYX): couldn't find masternode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeZenyxSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Zenyx specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeZenyxSpecific * MICRO, nTimeZenyxSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(ZENYX): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
