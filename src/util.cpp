...
const char * const BITCOIN_CONF_FILENAME = "zenyx.conf";
const char * const BITCOIN_PID_FILENAME = "zenyxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "zenyxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZenyxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZenyxCore
    // Mac: ~/Library/Application Support/ZenyxCore
    // Unix: ~/.zenyxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZenyxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZenyxCore";
#else
    // Unix
    return pathRet / ".zenyxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty zenyx.conf if it does not excist
...
{
    std::string strCopyrightHolders = strPrefix + strprintf(" %u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS), _(COPYRIGHT_HOLDERS_SUBSTITUTION));

    // Check for untranslated substitution to make sure Zenyx Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Zenyx Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Zenyx Core developers";
    }
